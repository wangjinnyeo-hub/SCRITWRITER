...
export function useKeyboardShortcuts() {
  ...
  useEffect(() => {
    ...
    commandRegistry.register({
      ...
      isEnabled: () => !!useUIStore.getState().activeEpisodeId && !!useProjectStore.getState().file,
      run: () => {
        const episodeId = useUIStore.getState().activeEpisodeId
        const file = useProjectStore.getState().file
        if (!episodeId || !file?.episodes.some(e => e.id === episodeId)) return
        let addedPlotBoxId: string | null = null
        useHistoryStore.getState().executeCommand({
          description: '플롯 박스 추가',
          execute: () => {
            useProjectStore.getState().addPlotBox(episodeId)
            const f = useProjectStore.getState().file
            const ep = f?.episodes.find(e => e.id === episodeId)
            if (ep?.plotBoxes.length) addedPlotBoxId = ep.plotBoxes[ep.plotBoxes.length - 1].id
          },
          undo: () => {
            if (addedPlotBoxId) useProjectStore.getState().removePlotBox(episodeId, addedPlotBoxId)
          },
        })
      },
    })
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (mod(e) && e.shiftKey && !e.altKey && (e.key === 'P' || e.key === 'p')) {
      e.preventDefault()
      const cmd = commandRegistry.get('plot.add')
