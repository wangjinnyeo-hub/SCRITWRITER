...
export function SettingsDialog({ open, onClose, initialSection }: SettingsDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[600px] max-h-[80vh] min-h-[400px] overflow-hidden flex flex-col p-0 gap-0 rounded border border-border bg-background shadow-none">
        <DialogHeader className="sr-only">
          <DialogTitle>설정</DialogTitle>
          <DialogDescription>일반, 단축키, 연출 관련 환경설정을 변경합니다.</DialogDescription>
        </DialogHeader>
        <SettingsEditSourceContext.Provider value="project">
          <SettingsDialogInner title="설정" onClose={onClose} resetToDefaults={resetToDefaults} initialSection={open ? initialSection ?? undefined : undefined}>
            {(section) => (
              <>
...
export function DefaultSettingsDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[600px] max-h-[80vh] min-h-[400px] overflow-hidden flex flex-col p-0 gap-0 rounded border border-border bg-background shadow-none">
        <DialogHeader className="sr-only">
          <DialogTitle>기본설정</DialogTitle>
          <DialogDescription>새 프로젝트 생성 시 적용할 기본 설정을 조율합니다.</DialogDescription>
        </DialogHeader>
        <SettingsEditSourceContext.Provider value="default">
          <SettingsDialogInner title="기본설정" onClose={onClose} resetToDefaults={resetToDefaults}>
            {(section) => (
              <>
                {section === 'general' && <GeneralSection />}
                {section === 'terminology' && <TerminologySection />}
...
function ShortcutsSection() {
  ...
  const fileCommands = commandRegistry.list().filter(c => c.category === 'file' && c.shortcut)
  ...
  return (
    <div className="space-y-4">
      <SettingGroup label="파일 단축키">
        {/* 타 프로그램 대비·통용 등 비교 서술 사용 금지 */}
