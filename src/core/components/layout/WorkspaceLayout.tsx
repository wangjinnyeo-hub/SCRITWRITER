...
export function WorkspaceLayout() {
  ...
  const closeFullView = useUIStore(state => state.closeFullView)
  ...
  return (
    <>
        ...
        {!activeEpisodeId ? (
          ...
          {editorLayoutMode === 'vertical' ? (
            ...
            <ResizablePanelGroup direction="horizontal" id="workspace-horizontal-reversed" onLayout={onLayoutSave('horizontal-reversed')}>
              ...
              <ResizableHandle />
              <ResizablePanel defaultSize={defaultLayoutByMode['horizontal-reversed'][1]} minSize={5} className="min-w-0 min-h-0">
            ...
            <ResizablePanelGroup key={`workspace-horizontal-${horizontalLayoutKey}`} direction="horizontal" id="workspace-horizontal" onLayout={onLayoutSave('horizontal')}>
              <ResizablePanel defaultSize={defaultLayoutByMode.horizontal[0]} minSize={5} className="min-w-0 min-h-0">
                <PlotEditor episodeId={activeEpisodeId} layoutMode="horizontal" useExternalDndContext />
              </ResizablePanel>
              <ResizableHandle />
      ...
      {fullViewOpen && activeEpisodeId && (
        <Suspense fallback={null}>
          <UnifiedFullView episodeId={activeEpisodeId} onClose={closeFullView} />
        </Suspense>
      )}
    </>
  )
