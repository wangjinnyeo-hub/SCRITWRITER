...
export function ScriptEditor({ episodeId, useExternalDndContext = false }: ScriptEditorProps) {
  ...
  const handleUpdateContent = (plotBoxId: string, unitId: string, content: string) => {
  ...
  const buildScriptUnitProps = useCallback(
    (
      ...
      onUnitDoubleClick: () => {
        const TYPE_CYCLE: Array<ScriptUnit['type']> = ['action', 'narration', 'background', 'dialogue', 'direction']
        const idx = TYPE_CYCLE.indexOf(unit.type)
        const nextType = TYPE_CYCLE[(idx + 1) % TYPE_CYCLE.length]
        const patch: Partial<ScriptUnit> = { type: nextType }
        if (nextType === 'dialogue') {
          patch.characterId = currentCharacterId ?? characters[0]?.id
        }
        updateScriptUnit(episodeId, plotBoxId, unit.id, patch)
        if (nextType === 'dialogue' && patch.characterId) setCurrentCharacter(patch.characterId)
      },
      ...overrides,
    }),
    [
  ...
  return (
    ...
    >
      ...
      {scriptCtx && <ContextMenu items={scriptCtx.items} position={scriptCtx.position} onClose={closeScriptContextMenu} />}
    </div>
  )
}
