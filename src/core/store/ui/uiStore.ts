...
export const useUIStore = create<UIState>((set, get) => ({
  ...
  getWorkspaceLayoutSnapshot: () => {
    const s = get()
    const confirmed = Array.isArray(s.confirmedPlotBoxIds) ? s.confirmedPlotBoxIds : []
    return {
  ...
  applyWorkspaceLayout: (layout) =>
    set((state) => ({
      ...
      workspacePanelSizes: layout.panelSizes
        ...
        : defaultPanelSizes(),
      ...(Array.isArray(layout.confirmedPlotBoxIds) ? { confirmedPlotBoxIds: layout.confirmedPlotBoxIds } : {}),
    })),
