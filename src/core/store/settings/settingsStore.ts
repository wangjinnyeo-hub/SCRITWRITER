...
interface SettingsState {
  ...
  setPdfPageSize: (size: PdfPageSize) => void
  setPdfCustomMarginMm: (margin: Partial<PdfMarginMm> | ((prev: PdfMarginMm) => PdfMarginMm)) => void
...
export const useSettingsStore = create<SettingsState>((set) => ({
  ...
  uiScalePercent: 120,
  pdfPageSize: 'a4',
  pdfCustomMarginMm: { top: 24, right: 20, bottom: 28, left: 20 },
  ...
  setPdfPageSize: (size) => set({ pdfPageSize: size }),
  setPdfCustomMarginMm: (margin) => set((state) => ({
    pdfCustomMarginMm: typeof margin === 'function' ? margin(state.pdfCustomMarginMm) : { ...state.pdfCustomMarginMm, ...margin },
  })),
