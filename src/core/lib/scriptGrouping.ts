...
/** 엑스트라 등 dialogueLabel 비교 시 undefined와 ''를 동일하게 취급 */
function sameDialogueLabel(a: string | undefined, b: string | undefined): boolean {
  return (a ?? '') === (b ?? '')
}
...
export function groupScriptUnitsByCharacter(
  ...
  while (i < units.length) {
    ...
    if (unit.type === 'dialogue' && unit.characterId) {
      ...
      i++
      while (i < units.length && units[i].type === 'dialogue' && units[i].characterId === charId && sameDialogueLabel(units[i].dialogueLabel, firstLabel)) {
        list.push(units[i])
        i++
      }
      groups.push({
