...
export function createSampleProject(): SWFile {
  ...
  const plot3Units: ScriptUnit[] = [
    ...
    u(1, 'narration', '단락 유형만 바꾸기 — 단락 왼쪽 핸들 우클릭으로 메뉴를 열고 유형을 선택한다.'),
    u(2, 'action', '# 유형 변경 — 단락 안에서 #(또는 ##, ###) 입력 후 스페이스. 현재 단락의 유형만 지정한 유형으로 바뀐다.'),
    u(3, 'narration', '유형 변경은 핸들 우클릭·#·/·@ 등을 사용한다.'),
  ]
  ...
  const episode: Episode = {
    ...
    plotBoxes: [
      ...
      { id: plotIds[2], order: 2, title: '3. 유형 변경', content: '핸들 우클릭, #, /, @', scriptUnits: plot3Units },
