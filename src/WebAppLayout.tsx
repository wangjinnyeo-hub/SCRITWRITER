...
interface SettingsDialogProps {
  open: boolean
  onClose: () => void
  initialSection?: 'general' | 'terminology' | 'shortcuts' | 'directions'
}
...
interface ExportDialogProps {
  ...
  initialIncludeProperties?: Record<string, unknown>
}

const SettingsDialog = lazy(() => import('@/components/settings/SettingsDialog').then(m => ({ default: m.SettingsDialog }))) as ComponentType<SettingsDialogProps>
const DefaultSettingsDialog = lazy(() => import('@/components/settings/SettingsDialog').then(m => ({ default: m.DefaultSettingsDialog }))) as ComponentType<DefaultSettingsDialogProps>
const ExportDialog = lazy(() => import('@/components/export/ExportDialog').then(m => ({ default: m.ExportDialog }))) as ComponentType<ExportDialogProps>
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="h-full min-h-0 flex flex-col bg-background">
      <div className="h-8 flex items-center flex-shrink-0 bg-[var(--panel-header)] pl-3 pr-2 border-b border-border">
        <span className="text-xs font-medium text-muted-foreground">Script Writer</span>
      </div>
      <Header />
